...
You can use your custom style by creating a corresponding file in the lucee/dump/skins folder. Check the folder for examples.",
	...
	void function init(boolean hasEndTag=false, component parent) {
		...
		if (server.lucee.version LT "4.2")
			throw message="Lucee 4.2 or newer is required for this tag";
	}
		...
		if (arguments.attrib.output == "browser" || arguments.attrib.output == true) {
			...
			if (arguments.attrib.format == "text") {
				...
				if (arguments.attrib.format == "simple") {
					...
					echo('<div id="-lucee-dump-#variables.dumpID#" class="-lucee-dump modern">');
...
/*						} */
	...
	string function simple(required struct meta, string title="") {
		...
		arrayAppend(variables.aOutput, '<style>.-lucee-dump-simple td { padding: 2px; }</style>');
		arrayAppend(variables.aOutput, '<table class="-lucee-dump-simple" cellpadding="2" cellspacing="0" border="1" #arguments.title#>');
	...
	string function writeCSS(required skinStyles) {
		...
		savecontent variable="local.style" trim=true { echo('
			...
			<style>
				.-lucee-dump.modern .nowrap  { white-space: nowrap; }
				.-lucee-dump.modern .bold    { font-weight: bold; }
				.-lucee-dump.modern .nobold  { font-weight: normal; }
				.-lucee-dump.modern .pointer { cursor: pointer; }
			');

			loop collection=styles item="local.value" index="local.key" {

				echo(".-lucee-dump.modern #key# {#value#}");
			}

			loop collection=colorScheme item="local.color" index="local.key" {

				echo(".-lucee-dump.modern .bgd-#lcase(key)# { background-color: #color.dark#; } ");
				echo(".-lucee-dump.modern .bgl-#lcase(key)# { background-color: #color.light#; } ");
			}
